...
def laser(n):
    terms = []
    for i in range(n):
        term = f"{{1 * [@delta{i}@ / 2]}} * Z{i} + {{1 * [@omega{i}@ * np.cos(@phi{i}@) / 2]}} * X{i} + {{-1 * [@omega{i}@ * np.sin(@phi{i}@) / 2]}} * Y{i}"
        terms.append(term)
    result = " + ".join(terms)
    return result
...
def laser_bs(n):
    ...
    local_lbs = {}
    for i in range(n):
        local_ubs[f"delta{i}"] = 20
        local_lbs[f"delta{i}"] = -20
        local_ubs[f"omega{i}"] = 2.5
        local_lbs[f"omega{i}"] = -2.5
