...
def solve_aligned(
    ...
    def build_obj(eqs, fixed_values):
        ...
        for i in range(nvar):
            if fixed_values[i] is None:
                ...
                # initialize evolution time variables
    ...
    if solver == "least_squares":
        ...
        info.append(["t_compile: ", end_time - start_time])
        ...
        info.append(["Accuracy: ", np.linalg.norm(f_sol[1:], 1)])
        if verbose > 0:
        ...
        info.append(["t_evol: ", [float(t) for t in gtime]])
        info.append(["instruction_value: ", gsol])
        gsol = np.array(gsol)
        return True, info, ali
    ...
    for x in range(mach.num_sites):
        ...
        for h, t in qs.evos:
            ...
            if not available:
                break
        if available:
            return align(i + 1, ali, qs, mach, solver, solver_args, verbose)
