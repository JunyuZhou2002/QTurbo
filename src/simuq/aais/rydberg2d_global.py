...
def generate_qmachine(n=3, inits=None):
    ...
    d = ins.add_local_variable(init_value=0, lower_bound=-20, upper_bound=20)
    ham_detuning = 0
    for i in range(n):
    ...
    o = ins.add_local_variable(init_value=0, lower_bound=0, upper_bound=2.5)
    p = ins.add_local_variable(init_value=0, lower_bound=-np.inf, upper_bound=np.inf)
    ham_rabi = 0
    for i in range(n):
