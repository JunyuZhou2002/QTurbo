...
def generate_qmachine(n=3, inits=None):
    ...
    for i in range(n):
        ...
        d = ins.add_local_variable(init_value=0.3, lower_bound=-20, upper_bound=20)
        # init = 1 for 20 size in table otherwise canot solve
        ins.set_ham(-d * noper[i])

    for i in range(n):
        L = rydberg.add_signal_line()
        ins = L.add_instruction("native", f"Rabi of site {i}")
        o = ins.add_local_variable(init_value=0.3, lower_bound=0, upper_bound=2.5)
        p = ins.add_local_variable(init_value=0.3, lower_bound=-np.inf, upper_bound=np.inf)
