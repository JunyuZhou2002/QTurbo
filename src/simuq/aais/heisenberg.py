...
def generate_qmachine(n=3, e=None):
    ...
    for i in range(n):
        ...
        amp = ins1.add_local_variable(init_value=0.3, lower_bound=-1, upper_bound=1)
        phase = ins1.add_local_variable(init_value=0.3, lower_bound=-2 * 3.1415926, upper_bound=2 * 3.1415926)
        ins1.set_ham(amp * (Expression.cos(phase) * ql[i].X + Expression.sin(phase) * ql[i].Y))

        ins2 = L.add_instruction("derived", "L{}_Z".format(i))
        amp = ins2.add_local_variable(init_value=0.3, lower_bound=-1, upper_bound=1)
    ...
    for q0, q1 in link:
        ...
        amp = ins.add_local_variable(init_value=0.3, lower_bound=-1, upper_bound=1)
